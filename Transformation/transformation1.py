...
# This path is where your cleaned CSVs will be saved
...
# We define the key columns for each table to handle duplicates/nulls correctly
tables = [
...
# 4. LOOP THROUGH TABLES (The "Ingestion Logic")
for t in tables:
    table_name = t["table"]
    key_cols = t["keys"]
    ...
    # B. TRANSFORMATION: Cleaning logic
...
# Only apply .str.strip() to columns that are actually strings
for t in tables:
    table_name = t["table"]
    key_cols = t["keys"]
    ...
    # --- IMPORTANT: These lines MUST be indented under 'for t in tables' ---
    # --- but NOT under 'for col in df.columns' ---
